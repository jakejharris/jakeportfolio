...
For an accepted request the route validates the slug, derives `views.<slug>`,
then uses one Sanity transaction to `createIfNotExists` at zero and increment
`count`. It does not read the post first. A lightweight bot User-Agent denylist
and best-effort two-second per-instance IP-and-slug throttle skip mutations and
...
later deployment after the seed has completed. Rollback is a redeploy with
`VIEW_WRITES_ENABLED=0`; existing counts remain readable.
